...
ls()
AL
AL$Y <- ifelse(AL$group=="D-", 0, ifelse(AL$group=="D0", 1, 2))
dim(AL)

AL.comp <- AL  #na.omit(AL)
dim(AL.comp)
head(AL.comp,2)

X <- AL.comp[, 2:15]
colnames(X) <- paste0("X", 1:14, sep="")
names(X)
head(X,2)

Y <- AL.comp$Y
dat.comb <- as.data.frame(cbind(X, Y))

actualData <- dat.comb
dat_3cat <- na.omit(actualData)

d <- 14
var <- paste0("X", 1:d)

dat_3cat$Xmax <- as.numeric(apply(dat_3cat[, var], 1, max))
dat_3cat$Xmin <- as.numeric(apply(dat_3cat[, var], 1, min))
...
# ptm <- proc.time()
# beta0 <- runif(d, -1, 1)#rep(1, d)
# opt <- optim(par=beta0, EHUM, dat=dat_3cat, d=d, method="L-BFGS-B", lower=-1, upper=1)
# beta.ehum.qn <- opt$par/sqrt(sum(opt$par^2))
# hum.ehum.qn <- -EHUM(dat_3cat, beta.ehum.qn, d)
# hum.ehum.qn
# print(proc.time()-ptm)
...
boot.fn.ehum <- function(dat, d, b){
  ...
  filename <- paste0("ehum_boot_AL_", b, ".RData")
...
SHUM <- function(dat, beta, d){
  ...
  I_sig <- function(x){
    return(1/(1+exp(-10*(x[1]-x[2]))) * 1/(1+exp(-10*(x[2]-x[3]))) )
  }
...
boot.fn.shum <- function(dat, d, b){
  ...
  filename <- paste0("shum_boot_AL_", b, ".RData")
...
NHUM <- function(dat, beta, d){
  ...
  I_norm <- function(x){
    return(pnorm(10*(x[1]-x[2])) * pnorm(10*(x[2]-x[3])))
  }
...
boot.fn.nhum <- function(dat, d, b){
  ...
  filename <- paste0("nhum_boot_AL_", b, ".RData")
...
boot.fn.pm <- function(dat, d, b){
  ...
  filename <- paste0("pm_boot_AL_", b, ".RData")
...
boot.fn.pmna <- function(dat, d, b){
  ...
  filename <- paste0("pmna_boot_AL_", b, ".RData")
...
boot.fn.mm <- function(dat, b){
  ...
  filename <- paste0("mm_boot_AL_", b, ".RData")
...
pdf("boxplot_AL_v2.pdf", width = 10, height = 10)
